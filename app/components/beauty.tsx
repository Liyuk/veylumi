...
export function MatchedProductRow({ match, saved, onToggle }: { match: ProductMatch; saved: boolean; onToggle: () => void }) {
  const { product } = match;
  return <div className="product-row"><div className="product-swatch" style={{ background: product.color }} /><div className="product-info"><span>{product.type} · {product.brand} · 匹配度 {match.score}%</span><strong>{product.name}</strong><small>{product.shade} · {product.skin} · {product.price}</small><em className="match-reason">{match.reason}</em><small className="match-caveat">{match.caveat}</small><a href={product.url} target="_blank" rel="noreferrer">查看商品 <ArrowTopRightIcon /></a></div><ProductLikeButton saved={saved} className="save-button" onToggle={onToggle} /></div>;
}
...
export function CatalogCard({ product, saved, onToggle, match }: { product: CatalogProduct; saved: boolean; onToggle: () => void; match?: ProductMatch }) {
  return <Card className="catalog-card"><div className="catalog-swatch" style={{ background: product.color }}><ProductLikeButton saved={saved} onToggle={onToggle} /><span>{match ? `${product.type} · 匹配 ${match.score}%` : product.type}</span></div><div className="catalog-copy"><small>{product.brand} · {product.region}</small><h3>{product.name}</h3>{match ? <><p>{match.reason}</p><small>{match.caveat}</small></> : <p>{product.tone} · {product.skin}</p>}<a href={product.url} target="_blank" rel="noreferrer">查看真实商品 <ArrowTopRightIcon /></a></div></Card>;
}
...
export function CatalogFilters({ referenceId, onReferenceChange, skin, onSkinChange, region, onRegionChange, products }: { referenceId: string; onReferenceChange: (value: string) => void; skin: "combination" | "dry" | "oily"; onSkinChange: (value: "combination" | "dry" | "oily") => void; region: "日韩" | "欧美" | "全部品牌"; onRegionChange: (value: "日韩" | "欧美" | "全部品牌") => void; products: CatalogProduct[] }) {
  return <div className="shade-match-controls"><Select.Root value={referenceId} onValueChange={onReferenceChange}><Select.Trigger aria-label="参考商品" placeholder="参考商品" /><Select.Content>{products.map((product) => <Select.Item key={product.id} value={String(product.id)}>{product.brand} · {product.shade}</Select.Item>)}</Select.Content></Select.Root><Select.Root value={skin} onValueChange={(value) => onSkinChange(value as typeof skin)}><Select.Trigger aria-label="肤质" /><Select.Content><Select.Item value="combination">混合皮</Select.Item><Select.Item value="dry">干皮</Select.Item><Select.Item value="oily">油皮</Select.Item></Select.Content></Select.Root><Select.Root value={region} onValueChange={(value) => onRegionChange(value as typeof region)}><Select.Trigger aria-label="品牌市场" /><Select.Content><Select.Item value="全部品牌">全部市场</Select.Item><Select.Item value="日韩">日韩</Select.Item><Select.Item value="欧美">欧美</Select.Item></Select.Content></Select.Root></div>;
}
