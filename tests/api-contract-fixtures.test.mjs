...
import recommendations from "../packages/api-contract/fixtures/recommendations.json" with { type: "json" };
...
test("cross-platform API fixtures retain stable success and error envelopes", () => {
  ...
  assert.equal(recommendations.data.ruleVersion, 1);
  assert.equal(recommendations.data.items[0].productId, 1);
});
