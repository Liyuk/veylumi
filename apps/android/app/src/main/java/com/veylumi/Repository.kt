...
class VeylumiRepository(private val api: VeylumiApiClient) {
    ...
    suspend fun analyze(image: ByteArray, filename: String, mimeType: String): AnalysisJob {
        if (!mimeType.startsWith("image/") || image.size > 10 * 1024 * 1024) throw VeylumiApiException("API_VALIDATION_ERROR", "Choose a JPG, PNG, or WEBP image smaller than 10 MB.")
    ...
    suspend fun poll(job: AnalysisJob, onStatus: (String) -> Unit): AnalysisJob { var current = job; while (current.status == "queued" || current.status == "running") { onStatus(current.status); kotlinx.coroutines.delay(900); current = api.job(current.jobId) }; return current }
