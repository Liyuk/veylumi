...
class AppViewModel : ViewModel() {
    ...
    fun toggleSaved(productId: Int) = save { current -> current.copy(savedProductIds = if (productId in current.savedProductIds) current.savedProductIds - productId else current.savedProductIds + productId) }
    fun updateSettings(settings: Settings) = save { it.copy(settings = settings) }
    fun addFeedback(kind: String) = save { current -> current.copy(feedback = current.feedback + buildJsonObject { put("id", java.util.UUID.randomUUID().toString()); put("kind", kind); put("createdAt", java.time.Instant.now().toString()) }) }
    private fun save(change: (StateSnapshot) -> StateSnapshot) = viewModelScope.launch { val data = mutableState.value.data ?: return@launch; runCatching { repository.saveWithMerge(change(data.state)) }.onSuccess { mutableState.value = mutableState.value.copy(data = data.copy(state = it)) }.onFailure { mutableState.value = mutableState.value.copy(error = it.message) } }
}
